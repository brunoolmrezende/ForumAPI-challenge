...
{
    public class RegisterCommentInvalidTokenTest : ForumClassFixture
    {
        private const string _endpoint = "comment";
        private readonly long _topicId;

        public RegisterCommentInvalidTokenTest(CustomWebApplicationFactory factory) : base(factory)
        {
        ...
        public async Task Error_Token_Empty(string culture)
        {
            ...
            var response = await DoPost(endpoint: $"{_endpoint}/{_topicId}", request: request, token: string.Empty, culture: culture);
        ...
        public async Task Error_Token_Invalid(string culture)
        {
            ...
            var response = await DoPost(endpoint: $"{_endpoint}/{_topicId}", request: request, token: "invalidToken", culture: culture);
        ...
        public async Task Error_Token_With_User_Not_Found(string culture)
        {
            ...
            var response = await DoPost(endpoint: $"{_endpoint}/{_topicId}", request: request, token: token, culture: culture);
